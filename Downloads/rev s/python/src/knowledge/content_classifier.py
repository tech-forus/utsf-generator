...
# Transit time / TAT sections — numeric values are days, NOT rates; must not
# bleed into rate extraction.  "10 transit days" is the canonical false positive.
_TRANSIT_DAYS_SIGNALS = {
    # Explicit compound phrases are the strongest signal
    "transit_phrase": re.compile(
        r'\b(?:transit\s*(?:time|days?)|delivery\s*(?:time|days?)|'
        r'days?\s*in\s*transit|lead\s*time|turnaround\s*time|'
        r'no\.?\s*of\s*days?|number\s*of\s*days?|days?\s*for\s*delivery)\b', re.I),
    # TAT / ETA as standalone tokens are very strong in header context
    "tat_eta":        re.compile(r'(?<!\w)(?:tat|eta)(?!\w)', re.I),
    # Cells that literally say "N days" / "N business days"
    "days_value":     re.compile(
        r'\b\d+\s*(?:business\s*|working\s*|transit\s*)?days?\b', re.I),
}
...
_WEIGHTS = {
    ...
    "TRANSIT_DAYS": {
        "transit_phrase":   5.0,   # "transit days" / "delivery time" is unambiguous
        "tat_eta":          4.0,   # TAT / ETA header = almost certain transit table
        "days_value":       1.5,   # "10 days", "3 business days" in cell values
    },
    "ZONE_MATRIX": {
...
class ContentClassifier:
    ...
    def _get_pattern(self, category: str, signal_name: str):
        mapping = {
            ...
            "TRANSIT_DAYS":  _TRANSIT_DAYS_SIGNALS,
        }
