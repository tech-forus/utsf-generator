...
class ZoneMapper:
    ...
    def _load_pincodes(self, path: str):
        ...
        if items is not None:
            for pin_str, vals in items:
                try:
                    pin   = int(pin_str)
                    zone  = str(vals[0]).upper().strip() if vals else ""
                    state = str(vals[1]).upper().strip() if len(vals) > 1 else ""
                    city  = str(vals[2]).upper().strip() if len(vals) > 2 else ""
