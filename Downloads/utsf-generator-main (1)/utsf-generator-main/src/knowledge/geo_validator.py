    ...
    def _load(self, pincodes_path: str):
        ...
        try:
            with open(pincodes_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            # Supports both compact dict {pin: [zone, state, city]} and legacy array format
            if isinstance(data, dict):
                for pin_str, vals in data.items():
                    if len(pin_str) == 6 and pin_str.isdigit():
                        zone  = str(vals[0]).strip().upper() if vals else ""
                        state = str(vals[1]).strip().upper() if len(vals) > 1 else ""
                        pin_int = int(pin_str)
                        self._pin_to_zone[pin_int]  = zone
                        self._pin_to_state[pin_int] = state
                        self._pin_to_data[pin_str]  = {"pincode": pin_str, "zone": zone, "state": state, "city": vals[2] if len(vals) > 2 else ""}
            else:
                for entry in data:
